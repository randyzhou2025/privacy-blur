...
const longCodePattern = /[A-Z0-9][A-Z0-9-]{9,}/i;
...
const amountValuePattern = /(?:[¥￥]\s*)?\d{1,3}(?:,\d{3})*(?:\.\d{1,2})?\s*(?:元|块|RMB|CNY)?/i;
const currencyValuePattern = /[¥￥]\s*\d{1,3}(?:,\d{3})*(?:\.\d{1,2})?|\d{1,3}(?:,\d{3})*(?:\.\d{1,2})?\s*(?:元|块|RMB|CNY)/i;
...
const rules: PrivacyRule[] = [
  ...
  },
  {
    ...
    risk: "medium",
    test: (text, compactText) => {
      if (!orderContextPattern.test(text) && !/[A-Z]/i.test(compactText)) return false;
      if (phonePattern.test(text) || idCardPattern.test(text)) return false;
      return longCodePattern.test(compactText) || /(^|[^\d])\d{10,18}(?!\d)/.test(compactText);
    },
  },
  {
    label: "金额疑似",
