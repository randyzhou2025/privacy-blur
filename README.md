...
生产构建使用相对资源路径，便于把 `dist/` 打包成可下载的本地版。离线包会把 JS/CSS 内联进根目录 `index.html`，让用户双击后可先使用手动打码；同时提供 macOS 启动脚本和 Windows 启动脚本，用于运行本机临时 HTTP 服务以支持完整本地 OCR。手机系统对本地网页、OCR worker 和 WASM 的限制不完全一致，如本地 OCR 被系统拦截，仍可使用在线入口，图片不会上传服务器。
